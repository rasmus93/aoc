...
int solve(VVI &a, int xx, int yy, int from) {

    queue<pt> q;
    q.push(pt{xx, yy, from});
...
int main() {
    ...
    int mx = 0;
    FOR(j, m) {
        mx = max(mx, solve(a, 0, j, 1));
        mx = max(mx, solve(a, m - 1, j, 3));
    }
    FOR(i, n) {
        mx = max(mx, solve(a, i, 0, 0));
        mx = max(mx, solve(a, i, n - 1, 2));
    }
    cout << mx;
    return 0;
}
