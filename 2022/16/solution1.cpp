...
//v,used,left -> res
map<tuple<int, ll, int>, int> xz;

int go(int v, ll used = 0, int left = 30) {
    tuple<int, ll, int> key = tuple(v, used, left);
    if (xz.count(key)) {
        return xz[key];
    }
    ...
    FOR(i, g2[v].size()) {
        ...
        res = max(
                res,
                go(to, used | (1ll << to), next_left) + d[to] * next_left
        );
    }
    xz[key] = res;
    return res;
...
VVI pre(int n, VI &flows) {
    ...
    FOR(i, flows.size()) {
        FOR(j, flows.size()) {
            ...
            g2[flows[i]].pb(mp(flows[j], dp[flows[i]][flows[j]]));
        }
    }
    return dp;
}
    ...
    FOR(i, all / 2 + 1) {
        ...
        FOR(j,flows.size()) {
            if (i & (1ll << j)) {
                ...
                elephant |= 1ll << flows[j];
            }
        }
        res2 = max(
                res2,
                go(m["AA"], me, 26) + go(m["AA"], elephant, 26)
        );
    }
    cout << res2;
    return 0;
